...
public class BTreeLeaf {
    ...
    public boolean next() {
        ...
        if (currentSlot >= contents.getNumRecords()) {
            ...
            return true;
        } else { // currentSlot < contents.getNumRecords() && !contents.getDataVal(currentSlot).equals(searchKey)
    ...
    public DirectoryEntry insert(Rid dataRid) {
        ...
        //
        // It is possible that searchKey is less than the first value in the leaf page (but greater than the dataVal of the corresponding directory entry)
        ...
        if (lastKey.equals(firstKey)) {
            ...
            contents.setFlag(newBlockId.blockNum()); // Current block's first key is the same as the overflow block's key
            // In general, a leaf block can contain a chain of overflow blocks. Each overflow block will be completely filled.
            // The records in the overflow chain will always have the same dataVal, which will always be the same as the dataVal of the first record in the non-overflow block.
            return null;
        } else {
