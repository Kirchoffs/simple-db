...
public class SetStringRecord implements LogRecord {
    ...
    // SETSTRING (4 bytes) - txNum (4 bytes) - fileName (variable) - blockNum (4 bytes) - offset (4 bytes) - val (variable)
    public static int writeToLog(LogManager logManager, int txNum, BlockId blockId, int offset, String val) {
        ...
        byte[] recordBytes = new byte[valPos + Page.maxLength(val.length())];
